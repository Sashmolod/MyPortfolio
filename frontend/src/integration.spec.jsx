...
vi.mock('framer-motion', () => {
  ...
  const Dummy = React.forwardRef(({ children, onAnimationComplete, ...props }, ref) => {
    React.useEffect(() => {
...
describe('Frontend Login-to-Dashboard Integration Flow', () => {
  ...
  it('navigates the user from LoginPage to AdminDashboard upon successful login', async () => {
    ...
    await waitFor(() => {
      expect(authApi.login).toHaveBeenCalledWith({ username: 'administrator', password: 'correct-pass' });
  ...
  it('allows switching dashboard tabs and toggling settings', async () => {
    ...
    expect(screen.getByText('Интерактивные функции и анимации')).toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(api.put).toHaveBeenCalledWith('/admin/settings', { enableSounds: false });
    });
  });
