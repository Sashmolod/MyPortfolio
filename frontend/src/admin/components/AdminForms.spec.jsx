...
describe('Admin Forms components', () => {
  ...
  describe('SkillForm', () => {
    ...
    it('renders Edit Skill mode when item is provided', async () => {
      ...
      render(
        <SkillForm item={item} onSaveData={mockSave} onCancel={mockCancel} />
      );
  ...
  describe('ProjectForm', () => {
    ...
    it('handles image file uploads and updates URL', async () => {
      vi.mocked(api.post).mockResolvedValueOnce({
        data: { url: '/uploads/my-proj.png' },
      });
      const { container } = render(
        <ProjectForm onSaveData={vi.fn()} onCancel={vi.fn()} />
      );
      ...
      expect(
        screen.getByDisplayValue('/uploads/my-proj.png')
      ).toBeInTheDocument();
  ...
  describe('HeroForm', () => {
    ...
    it('handles avatar upload', async () => {
      ...
      const { container } = render(
        <HeroForm onSaveData={vi.fn()} onCancel={vi.fn()} />
      );
      ...
      expect(
        screen.getByDisplayValue('/uploads/avatar.jpg')
      ).toBeInTheDocument();
    });
    ...
    it('attempts to trigger sketch conversion', async () => {
      ...
      await waitFor(() => {
        expect(api.post).toHaveBeenCalled();
        expect(window.toast).toHaveBeenCalledWith(
          'Avatar converted to sketch successfully',
          'success'
        );
      });

      expect(
        screen.getByDisplayValue('/uploads/sketch.png')
      ).toBeInTheDocument();
  ...
  describe('SocialLinkForm', () => {
    it('validates required platform and URL, then saves correctly', async () => {
      ...
      const platformInput = screen.getByPlaceholderText(
        'Platform (e.g. GitHub, Reddit)'
      );
