...
export default function SkillForm({ item, onSaveData, onCancel }) {
  const [form, setForm] = useState(() => {
    if (item)
      return pick(item, ['name', 'icon', 'description', 'level', 'sortOrder']);
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    await onSaveData(
      pick(form, ['name', 'icon', 'description', 'level', 'sortOrder'])
    );
    setSaving(false);
  };
      ...
      <form onSubmit={handleSubmit} noValidate>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          <div>
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
              <input
                ...
                onChange={(e) =>
                  setForm({
                    ...form,
                    icon: e.target.value.trim().toLowerCase(),
                  })
                }
            ...
            >
              Ключи: javascript, typescript, react, nodejs, python, postgresql,
              mongodb, docker, git, css3, html5, nestjs, redux, graphql, linux,
              figma, swift, kotlin, rust, go, vuejs, angularjs, nextjs, nuxtjs
            </p>
          </div>
          <input
