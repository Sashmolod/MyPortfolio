...
const SOCIAL_ICONS = {
  ...
  twitter: {
    viewBox: '0 0 24 24',
    paths: (
      <path d="M4 4l7.5 10L4 20h3l6-8 5 8h3l-8-11L20 4h-3l-5 7-4-7H4z" />
    )
  },
  bluesky: {
    viewBox: '0 0 24 24',
...
function SocialIcon({ name, size = 20 }) {
  ...
  useEffect(() => {
    ...
    fetch(`https://cdn.jsdelivr.net/npm/simple-icons@v11/icons/${slug}.svg`)
      ...
      .then(svgText => {
        ...
        if (match) {
          const pathsContent = svgText.substring(svgText.indexOf('>') + 1, svgText.lastIndexOf('</svg>'));
  ...
  return (
    ...
    >
      <text x="50%" y="55%" textAnchor="middle" fill="currentColor" fontSize="12" fontWeight="bold">
        {name?.[0]?.toUpperCase() || '?'}
      </text>
    </svg>
  );
...
export default function Hero({ data }) {
  ...
  const linksToRender = parsedLinks.length > 0
    ? parsedLinks
    : [
