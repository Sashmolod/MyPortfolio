...
vi.mock('framer-motion', () => {
  ...
  const DummyDiv = React.forwardRef(({ children, animate, initial, exit, transition, whileInView, viewport, ...props }, ref) => {
    return React.createElement('div', { ref, ...props }, children);
  });

  const DummyForm = React.forwardRef(({ children, animate, initial, exit, transition, whileInView, viewport, ...props }, ref) => {
    return React.createElement('form', { ref, ...props }, children);
  });

  const DummyP = React.forwardRef(({ children, animate, initial, exit, transition, whileInView, viewport, ...props }, ref) => {
...
describe('ContactForm Component', () => {
  ...
  it('displays validation errors on empty submission', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Please solve the math puzzle')).toBeInTheDocument();
    });
  ...
  it('submits successfully with valid data and calls post API', async () => {
    ...
    fireEvent.change(messageInput, { target: { value: 'This is a long message for testing.' } });
    fireEvent.change(captchaInput, { target: { value: '8' } });
    ...
    expect(window.toast).toHaveBeenCalledWith('Message sent successfully!', 'success');
  ...
  it('handles submission errors and displays server captcha errors', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Message'), { target: { value: 'This is a long message for testing.' } });
    fireEvent.change(screen.getByPlaceholderText('Ответ'), { target: { value: '9' } });

    const form = screen.getByPlaceholderText('Name').closest('form');
    fireEvent.submit(form);

    await waitFor(() => {
      expect(window.toast).toHaveBeenCalledWith('Incorrect captcha answer', 'error');
