...
export default function DoodleControls({ active, setActive, color, setColor, brushWidth, setBrushWidth, onUndo, onClear }) {
  ...
  // Brush presets
  const presets = [
    { name: 'Pencil', color: 'rgba(74, 85, 104, 0.85)', width: 3, label: '✏️ Pencil' },
        ...
        {minimized ? (
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontWeight: 'bold', fontSize: '14px', userSelect: 'none' }}>
                🎨 Doodles / Рисование
              </span>
              <button
            ...
            <button
              ...
              style={{
                ...
                justifyContent: 'center',
                gap: '6px',
                backgroundColor: active ? 'rgba(229, 62, 62, 0.15)' : 'transparent',
                borderColor: active ? 'var(--accent)' : 'var(--text)'
              }}
            >
              {active ? '📴 Disable Drawing / Навигация' : '✏️ Enable Drawing / Рисовать'}
            </button>
              ...
              {active && (
                ...
                >
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <span style={{ fontSize: '12px', opacity: 0.8, userSelect: 'none' }}>Select Tool:</span>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                      {presets.map((p) => {
                        const isSelected = color === p.color && brushWidth === p.width;
                        return (
                          <button
                            key={p.name}
                            onClick={() => {
                            ...
                            style={{
                              ...
                              padding: '5px 8px',
                              textAlign: 'left',
                              border: isSelected ? '2px solid var(--accent)' : '1px solid var(--text)',
            ...
            {active && (
              <p style={{ margin: 0, fontSize: '10px', opacity: 0.7, textAlign: 'center', userSelect: 'none' }}>
                * Link clicks are disabled while drawing. Turn off to click links.
              </p>
            )}
          </motion.div>
        )}
